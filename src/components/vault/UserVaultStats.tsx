...
export function UserVaultStats({ stats, isLoading }: UserVaultStatsProps) {
  ...
  if (!stats) {
    return (
      <div className="text-center py-4">
        <p className="text-sm sm:text-base text-muted-foreground">{t('vault.yourStats.noStats')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {/* Percentile Banner */}
      <div className="p-3 sm:p-4 rounded-lg bg-gradient-to-r from-primary/10 via-accent/10 to-primary/10 border border-primary/20">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="p-1.5 sm:p-2 rounded-full bg-primary/20">
            <TrendingUp className="h-4 w-4 sm:h-5 sm:w-5 text-primary" />
          </div>
          <div>
            <p className="text-xs sm:text-sm font-medium text-primary">{percentileMessage}</p>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        ...
        <div className="p-2 sm:p-3 rounded-lg bg-orange-500/10 border border-orange-500/20 text-center">
          <Flame className="h-4 w-4 sm:h-5 sm:w-5 mx-auto text-orange-400 mb-1" />
          <div className="text-xl sm:text-2xl font-bold text-orange-400">{stats.current_streak}</div>
        ...
        {/* Predictions */}
        <div className="p-2 sm:p-3 rounded-lg bg-purple-500/10 border border-purple-500/20 text-center">
          <Target className="h-4 w-4 sm:h-5 sm:w-5 mx-auto text-purple-400 mb-1" />
        ...
        {/* Badges */}
        <div className="p-2 sm:p-3 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-center">
          <Award className="h-4 w-4 sm:h-5 sm:w-5 mx-auto text-cyan-400 mb-1" />
          <div className="text-xl sm:text-2xl font-bold text-cyan-400">{stats.badges.length}</div>
          <div className="text-[8px] sm:text-[10px] text-muted-foreground uppercase tracking-wide">
      ...
      {stats.badges.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-xs sm:text-sm font-medium">{t('vault.yourStats.earnedBadges')}</h4>
            <Button
              variant="ghost"
              size="sm"
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2">
            {stats.badges.slice(0, 6).map((badge) => (
              <div
                key={badge.id}
                className={`flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full ${vaultService.getRarityBgColor(badge.rarity)} border border-current/20`}
                title={badge.description}
              >
                <span className="text-sm sm:text-base">{badge.icon}</span>
                <span className={`text-[10px] sm:text-xs font-medium ${vaultService.getRarityColor(badge.rarity)}`}>
                  {badge.name}
                </span>
              </div>
            ))}
      ...
      {stats.genres_explored.length > 0 && (
        <div>
          <h4 className="text-xs sm:text-sm font-medium mb-2">{t('vault.yourStats.genresExplored')}</h4>
      ...
      <div className="p-2 sm:p-3 rounded-lg bg-primary/5 border border-primary/10">
        ...
        <Progress value={getNextBadgeProgress(stats)} className="h-1 sm:h-1.5" />
      </div>
    </div>
  );
}
