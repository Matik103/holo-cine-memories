  ...
  if (champions.length === 0) {
    return (
      <div className="text-center py-8">
        <Crown className="h-8 w-8 mx-auto mb-2 text-primary/50" />
        <p className="text-muted-foreground">{t('vault.champions.empty')}</p>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              {index === 0 && (
                <Badge className="text-[10px] px-1.5 py-0 h-4 bg-yellow-500/20 text-yellow-400 border-yellow-500/30">
              ...
              {champion.badges.length > 0 && (
                <span className="flex items-center gap-1">
                  {champion.badges.slice(0, 3).map((badge, i) => (
                    <span key={i} className="text-sm">{badge.icon}</span>
                  ))}
